...
const fetchProjectEmployee = (userId) => {
  ...
  const query = `SELECT project_id, project_name, click, project_description, project.users_id, start_date, end_date, date_created, project.status_id,
    ...
    LEFT JOIN users
    ON project.users_id = users.users_id
    WHERE project.employee_id LIKE (?)`;
  return {
    sql: query,
    values: [`%${userId}%`],
  };
...
const updateDoneProjectStatus = (statusId, projectId) => {
  const query = `UPDATE project_status
  SET status_id= ?
  WHERE project_id = ?`;
  return {
    sql: query,
...
const fetchAdminTable = () => {
  const query = `
    SELECT project.project_id, project_name, start_date, end_date, date_created,
      ...
      LEFT JOIN status
      ON project.status_id = status.status_id GROUP BY project.project_id
  `;
  return {
    sql: query,
    values: [],
