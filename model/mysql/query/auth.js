...
const update = (id, { fname, mname, lname, nname, bday, password }) => {
  const query = `UPDATE users
    SET first_name=?, last_name=?, middle_name=?, nick_name=?, birthday=?, password=?
    WHERE users_id=?`;
  return {
    sql: query,
    values: [fname, lname, mname, nname, bday, password, id],
  };
};
...
const getUsersByUserLevel = (userLevel) => {
  const query = `SELECT users_id, first_name, last_name, middle_name, birthday, users.user_level_id, email, user_name, password,is_confirmed, address, contact_number,created_at, updated_at, user_level.user_level_name, user_level.user_level_acc
    FROM users
    LEFT JOIN user_level
    ON users.user_level_id = user_level.user_level_id
